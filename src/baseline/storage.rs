...
pub fn load(baseline_dir: &Path, name: &BaselineName) -> std::result::Result<Baseline, BaselineError> {
    let path = baseline_dir.join(format!("{}.json", name));
    load_file(&path)
}

/// Load a baseline from a file path.
pub fn load_file(path: &Path) -> std::result::Result<Baseline, BaselineError> {
...
pub fn save(
    ...
    report: &BenchReport,
    cli: &BenchCli,
) -> std::result::Result<(), BaselineError> {
    ...
    {
        let file =
            File::create(&temp_path).map_err(|e| BaselineError::CreateTemp { path: temp_path.clone(), source: e })?;
        ...
        writer
            .get_ref()
            .sync_all()
            .map_err(|e| BaselineError::Sync { path: temp_path.clone(), source: e })?;
    }
